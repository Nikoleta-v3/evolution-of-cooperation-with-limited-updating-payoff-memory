...
In each case, we assumet there are only two strategies present in the population (a resident and a mutant strategy). 
...
\begin{equation} \label{Eq:Condition_OneInteractionExpectedPayoff}
  q<1 - \frac{c}{\delta b}
\end{equation}
Interestingly, this condition is the same as condition~\eqref{Eq:Condition_PerfectMemory} for the case of perfect memory when $N\! \rightarrow\! \infty$.
...
\begin{figure}[t!]
  ...
  \caption{{\bf Evolution of direct reciprocity for different parameter values for memory-one strategies.}
  To further explore the robustness of our results in the case of memory-one
...
\section{Further repeated games; Snowdrift game}
\label{section:furtherrepeatedgames}

So far, we have discussed the repeated donation game, which is only one example
of social dilemmas. In the following section, we explore how our methodology can
be applied to other games, with a focus on the snowdrift game.

Similar to the donation game, the snowdrift game is a two-player game where
individuals choose to cooperate (\(C\)) or defect (\(D\)) each round.
Cooperation yields a benefit \(b\) to both players, incurring a cost \(c\) if
the opponent defects, but only \(c/2\) if the opponent cooperates. This results
...
\end{align}\label{eq:snowdrift_payoff}
...
limited memory scenarios. In Section~\ref{section:simulation_results_snowdrift},
we present simulation results for all six cases considered in this study.

\subsection{Stochastic stability of cooperation.}\label{section:stochastic_stability_snowdrift}

{\bf Perfect payoff memory.} We have already discussed the general setup for the
case of perfect memory in Section~\ref{section:perfect_memory}. Here, we derive
the conditions under which cooperation is stochastically stable for the
snowdrift game. As a reminder, we suppose that there is a single mutant,
...
\end{equation}
For such a strategy to be feasible we require $q\!>\!0$, which implies $\delta
\!>\! \big((N\!-\!2)b+c\big)/\big(N(c/2)\big)$. In particular, in the limit of
large populations \(N \!\rightarrow\! \infty\), we obtain that cooperation is
stochastically stable if  \(q \!<\! 1\! -\! c/(2\delta b)\). \\


\noindent
{\bf Limited payoff memory.} Once again, we examine
the scenario where a single \alld{} mutant can potentially invade a resident
population of \gtft{} players. However, this time, we consider that individuals
...
this result, we first need to discuss the difference between the two games. The
difference between the two games lies in the relationship between the possible
payoffs. More specifically, in the donation game, the payoffs satisfy the
relationship \(T > R > P > S\), whereas in the snowdrift game, it's \(T > R > S
> P\).
Now, note that for the above condition, the relationship between all the
possible payoffs is not important; what is important is the relationship between
all the feasible pairs of payoffs. In a population of \gtft{} strategies with a
single mutant of \alld{}, the feasible pairs of payoffs are \((R, T)\), \((R,
P)\), \((S, T)\), and \((P, P)\). In both games, the payoff \(T\) is higher than
\(S\) and \(R\), and \(R\) is higher than the punishment payoff, thus the
condition remains unchanged.

\subsection{Simulation results for reactive strategies}\label{section:simulation_results_snowdrift}

To investigate whether the previous static observations accurately depict the
dynamics of evolving populations, we once again rely on simulations. Firstly, we
conducted separate simulations for perfect and limited payoff memory. In each
...
evolving cooperation rates are high for all the cases. Even in the limited
payoff memory for low benefit, the evolving cooperation is at 71\%. Thus, the
...
snowdrift game with either a low or high benefit of cooperation. In the top
panels, individuals have perfect payoff memory, meaning they consider all their
past interactions against all population members, taking every turn of each
repeated game into account. In the bottom panels, individuals have limited
payoff memory, considering only their very last interaction against one specific
population member, and taking into account only one round of the repeated game.
...
Furthermore, to understand the impact of payoff memory on the snowdrift game
even more, we performed simulations for the four additional scenarios considered
in this work. As a reminder, in the first scenario, players recall the payoffs
they obtained in the last two rounds against a single co-player. In the second
scenario, players recall their last-round payoffs against two co-players. In the
third scenario, they recall the last two rounds against two co-players. Finally,
in the last scenario, players update based on the average payoff they receive
over all rounds with a single co-player (further extensions are possible, but we
do not explore them here). In Supplementary
Fig.~\ref{fig:cooperation_rate_all_updating_payoffs_snowdrift}, we show the
evolving cooperation rates for all these scenarios, as well as for the limited
and perfect memory cases.

We observe that main pattern of our results remains unchanged: the perfect
memory case always overestimates the cooperation rate. Compared to the baseline
case with a single round recalled, even a minimal increase in the players'
payoff memory further increases the cooperation rate. In all four scenarios with
extended memory, we observe similar cooperation rates, and they quickly approach
the rates observed under perfect memory. This happens at a faster rate compared
to the donation game.
...
\begin{figure}[t]
  ...
  Simulations are run either for a comparably low benefit of cooperation~($b/c\!=\!2$), or for a high benefit~($b/c\!=\!10$). 
  We observe that perfect memory always yields the highest cooperation rate, and limited memory yields the lowest.
  However, in the snowdrift game, the difference between the two extreme scenarios is not large, as we have already discussed.
  When individuals take into account a bit more information—cases (iii) to (vi)—evolving
  cooperation rates quickly approach the optimum of perfect memory. This increase happens faster compared to the donation game.

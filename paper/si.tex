...
So far, we have discussed the prisoner's dilemma. However, this is only one
example of social dilemmas. Further examples of social dilemmas include the
snowdrift game~\cite{doebeli:Ecology:2005}, the stag hunt
game~\cite{pacheco:ProcB:2009}, and the public goods
game~\cite{pinheiro:PLoSCompBio:2014}. In the following, we discuss how our
methodology can be applied to other games. More specifically, here we focus on
the snowdrift game.

Similarly to the prisoner's dilemma, the snowdrift game is a two-player game. In
each round, individuals can choose whether to cooperate ($C$) or defect ($D$).
Here, cooperation yields a benefit \(b\) to the cooperator as well as to the
opposing player and incurs a cost \(c\) if the opponent defects, but only a cost
\(c/2\) if the opponent cooperates. This results in the following payoffs:
...
If \(b>c>0\), as before, then cooperation is a better strategy than defection if the opponent
plays defection. On the other hand, if the opponent plays C, then D is still the best
response. Thus, both strategies can invade when rare, resulting in a mixed
evolutionarily stable state at which the proportion of cooperators is
\(1-c/(2b-c)\). It is important to note that in this state, the population
payoff is smaller than it would be if everybody played C; hence, the SD still
represents a social dilemma.


In the following, we discuss our methodology for the snowdrift game. We start by
considering the case of perfect memory, and then we extend our analysis to the
case of limited memory. More specifically, we look again at which conditions
cooperation is stochastically stable for the two extreme cases of perfect and
limited memory. We then present simulation results for all the six cases
...
This condition is identical to condition~\eqref{Eq:TransitionRatioSimple} for

...
\begin{figure}[t]
  ...
  ({\it vi}) based on the expected payoff of one interaction.
...
\begin{equation}\label{eq:appendix_fixation_probability}
    \rho_{M} = \frac{1}{1+\sum\limits_{i=1}^{N-1}\prod\limits_k^i \frac{\lambda^-_k}{\lambda^+_k}},
\end{equation}
...
strategies of a mutant and a resident, the expected payoffs \(\pi_\rolemodel\) and
\(\pi_M\) are given by,

\begin{equation} \label{Eq:ExpPay}
  \begin{array}{lcrcr}
  \displaystyle \pi_\rolemodel & = &\displaystyle \frac{N\!-\!k\!-\!1}{N-1}\cdot \langle\mathbf{v}(s_R,s_R),\mathbf{u}\rangle	&+	&\displaystyle\frac{k}{N-1}\cdot \langle\mathbf{v}(s_R,s_M),\mathbf{u}\rangle,\\[0.5cm]
...
\begin{align}\label{eq:perfect_memory_lambdas}
  \lambda^-_k \!=\!\varphi(\pi_M, \pi_\rolemodel) \quad \text{ and } \quad \lambda^+_k \!=\!\varphi(\pi_\rolemodel, \pi_M).
\end{align}
...
\lambda^+_k=\frac{N\!-\!k}{N} \cdot \frac{k}{N} \cdot \sum_{u_{R},u_{M}\in\mathcal{U}} x(u_{R},u_{M}) \varphi(u_{R},u_{M}) \quad \text{and} \quad
\lambda^-_k=\frac{N\!-\!k}{N} \cdot \frac{k}{N} \cdot \sum_{u_{R},u_{M}\in\mathcal{U}} x(u_{R},u_{M}) \varphi(u_{M},u_{R}).
\end{align}
